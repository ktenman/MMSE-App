...
export class TestEntityComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.testEntities = dataFromBody;
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ITestEntity[] | null): ITestEntity[] {
    ...
    if ('prev' in this.links) {
      const testEntitiesNew = this.testEntities ?? [];
      if (data) {
        for (const d of data) {
          if (testEntitiesNew.map(op => op.id).indexOf(d.id) === -1) {
            testEntitiesNew.push(d);
          }
        }
      }
      return testEntitiesNew;
    }
    return data ?? [];
  }
