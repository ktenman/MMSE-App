...
/**
 * Performance test for the PatientProfile entity.
 */
public class PatientProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all patientProfiles").get("/api/patient-profiles").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new patientProfile")
                        .post("/api/patient-profiles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                    "\"patientId\": \"SAMPLE_TEXT\"" +
                                    ", \"name\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_patientProfile_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created patientProfile").get("${new_patientProfile_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created patientProfile").delete("${new_patientProfile_url}").headers(headers_http_authenticated))
                .pause(10)
        );
    ScenarioBuilder users = scenario("Test the PatientProfile entity").exec(scn);
